...
type PushService interface {
	GetConfigs(userID uint) ([]*models.PushConfig, error)
	GetConfig(id uint) (*models.PushConfig, error)
	CreateConfig(config *models.PushConfig) error
	UpdateConfig(config *models.PushConfig) error
	DeleteConfig(id uint) error
	Send(configID uint, data map[string]interface{}) error
	GetHistory(configID *uint, page, pageSize int) ([]*models.PushHistory, int64, error)
	GetConfigFields(configID uint) ([]*models.FieldSchema, error)
...
func (s *pushService) GetConfig(id uint) (*models.PushConfig, error) {
	var config models.PushConfig
	err := s.db.First(&config, id).Error
	return &config, err
}

func (s *pushService) CreateConfig(config *models.PushConfig) error {
	return s.db.Create(config).Error
}

func (s *pushService) UpdateConfig(config *models.PushConfig) error {
	return s.db.Save(config).Error
}

func (s *pushService) DeleteConfig(id uint) error {
	return s.db.Delete(&models.PushConfig{}, id).Error
}
...
func (s *pushService) GetHistory(configID *uint, page, pageSize int) ([]*models.PushHistory, int64, error) {
	var history []*models.PushHistory
	var total int64

	query := s.db.Model(&models.PushHistory{})
...
func (s *pushService) GetConfigFields(configID uint) ([]*models.FieldSchema, error) {
...
func (s *pushService) UpdateConfigFields(configID uint, fieldIDs []uint) error {
	return s.db.Transaction(func(tx *gorm.DB) error {
		// 删除旧关联
		if err := tx.Where("config_id = ?", configID).Delete(&models.ConfigFieldRelation{}).Error; err != nil {
			return err
		}
		// 创建新关联
		for _, fieldID := range fieldIDs {
			if err := tx.Create(&models.ConfigFieldRelation{
				ConfigID: configID,
				FieldID:  fieldID,
			}).Error; err != nil {
				return err
